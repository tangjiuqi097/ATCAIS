...
class HybridTaskCascade(CascadeRCNN):
    ...
    def simple_test_post(self, img_meta, proposals=None, rescale=False):
        x = self.feature
        proposal_list = self.simple_test_rpn(
            x, img_meta, self.test_cfg.rpn) if proposals is None else proposals

        semantic_feat=self.semantic_feat
    ...
    def extract_feat_pre(self, img):
        self.feature = self.extract_feat(img)
        if self.with_semantic:
            _, semantic_feat = self.semantic_head(self.feature)
        else:
            semantic_feat = None
        self.semantic_feat=semantic_feat

    def extract_roi_featrue(self,rois):
        # only for single level feature, i.e. the max size level of FPN
        x=self.feature
        semantic_feat=self.semantic_feat
        bbox_roi_extractor = self.bbox_roi_extractor[-1]

        bbox_feats = bbox_roi_extractor.forward_singe_feature(
            x[:len(bbox_roi_extractor.featmap_strides)], rois)
        if self.with_semantic and 'bbox' in self.semantic_fusion:
    ...
    def forward_train_on_trackingdata(self,
                      img_meta,
                      gt_bboxes,
                      gt_labels,
                      gt_bboxes_ignore=None,
    ...
    def forward_train_frame_test(self,
                                 img_meta,
                                 gt_bboxes,
                                 gt_labels,
                                 gt_bboxes_ignore=None,
    ...
    def forward_train_frame(self,
        ...
        for i in range(self.num_stages):
            ...
            # bbox head forward and loss
            loss_bbox, rois, bbox_targets, bbox_pred = \
                self._bbox_forward_train_frame(
                    i, x, sampling_results, gt_bboxes, gt_labels,
                    rcnn_train_cfg, semantic_feat)
            roi_labels = bbox_targets[0]
    ...
    def _bbox_forward_train_frame(self,
                                  stage,
                                  x,
                                  sampling_results,
                                  gt_bboxes,
