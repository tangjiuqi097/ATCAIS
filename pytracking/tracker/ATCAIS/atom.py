...
from .det_mmdet import Detector
...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, state,depth_im=None, *args, **kwargs):
        ...
        self.use_det=True
        if self.use_det:
            # self.detector=Detector('/home/tangjiuqi097/research/pytracking/pytracking/detection/configs/e2e_mask_rcnn_X_101_32x8d_FPN_1x_test.yaml')
            self.detector=Detector()
            self.detector.new_image=True
            self.detector.initialize(image[:,:,::-1],self.pos,self.target_sz)


            if depth_im is None:
                if isinstance(kwargs['color'],str):
                    color_path=kwargs['color']
                    depth_path=color_path.replace("color",'depth')
                    depth_path = depth_path.replace("jpg", "png")
                    depth=Image.open(depth_path)
                    depth_im=np.array(depth)

            if not self.detector.weak_det:
                first_frame_prediction=self.detector.first_frame_prediction
        ...
        self.use_distractor_tracker=True
        if self.use_distractor_tracker:

            high_confidence_distractor_ind=self.detector.first_frame_distractors_boxes.get_field("scores")>0.7
            high_confidence_distractor =self.detector.first_frame_distractors_boxes[high_confidence_distractor_ind]
            # self.detector.show_prediction(image=image, predictions=high_confidence_distractor)
            top_k=2
            num_distractor_tracker=min(top_k,len(high_confidence_distractor))

            _,inds=torch.topk(high_confidence_distractor.get_field("scores"),num_distractor_tracker)
            self.distractor_trackers=[]
            for ind in inds:
                ...
                distractor_tracker = tracker_class(parameters)
                state=high_confidence_distractor[ind].convert("xywh").bbox
                state=state.reshape(-1)
                distractor_tracker.initialize( image, state)
                self.distractor_trackers.append(distractor_tracker)


        self.last_pos=self.pos.clone()
        self.last_target_sz=self.target_sz.clone()
        self.first_frame_target_sz = self.target_sz.clone()

        self.first_frame_filter = self.filter.clone()
        # self.init_optimization_distractor(train_x, init_y)
        self.first_frame_target_sz = self.target_sz.clone()
        self.first_frame_target_scale = self.target_scale
    ...
    def track(self, image,depth_im=None, *args, **kwargs):

        self.frame_num += 1

        if self.frame_num==950:
            print("debug")
...
####################################################################################################################
        self.pos_redet, self.flag_redet, self.conf_redet,_=self.redet(im,self.first_frame_target_scale)

        if self.use_det:
            try:
                if not self.detector.weak_det:
                    state=self.tracking_by_detection(image,depth_im)
                else:
                    state=None
                    self.tracking_by_weak_detection(image,depth_im)
            ...
            frame_state = BoxList(target_bbox, [image_sz[1], image_sz[0]], mode="xyxy")
            prediction_target = self.detector.compute_mask_from_box(image[:, :, ::-1], frame_state)


            if prediction_target is not None and len(prediction_target)>0 :
                depth_value,mask_area,valid_ratio,outlay_ratio = self.compute_depth_value_and_mask_area(depth_im, prediction_target)
            ...
            if prediction_target is None or prediction_target.extra_fields['mask'].sum() ==0:
                flag="not_found"
                self.pos=self.last_pos
                self.target_sz=self.last_target_sz

            if self.conf_redet>0.5:
                if len(self.detector.distractors_boxes)<200:
                    self.detector.update_distractors_boxes(image,self.pos_redet,self.first_frame_target_sz,self.prediction_det)


            if self.conf_redet>0.7 and len(self.prediction_det)>0:
                target_bbox = torch.cat(
                    (self.pos_redet[[1, 0]] - (self.first_frame_target_sz[[1, 0]] - 1) / 2, self.pos_redet[[1, 0]] + (self.first_frame_target_sz[[1, 0]] - 1) / 2))

                bbox = self.prediction_det.bbox

                iou = self.detector.compute_iou(bbox, target_bbox)
                iou = iou.reshape(-1)
                iou_max, ind = torch.max(iou, dim=0)

                target_area=self.first_frame_target_sz[0]*self.first_frame_target_sz[1]
                det_area=self.prediction_det.area()[ind]

                if iou_max>0.6  and (0.7<target_area/det_area<1.3)  :
                    state, [L, S], _ = self.detector.compute_box_from_mask(self.prediction_det[ind])
                    state = state.reshape(-1)
                    self.detector.last_S = S
                    self.detector.last_L = L
                    self.pos = torch.Tensor([state[1] + (state[3] - 1) / 2, state[0] + (state[2] - 1) / 2])
                    self.target_sz = torch.Tensor([state[3], state[2]])
                else:
                    self.pos=self.pos_redet
                    self.target_sz=self.first_frame_target_sz
...
#####################################################################################################################
        ...
        # self.overlay_boxes(image,new_state)
        return new_state.tolist(),torch.max(s[scale_ind,...]).item()

    def overlay_boxes(self,image, state, image_file=None, tracker_name='test'):
        import cv2
        import os
        ...
        seq_name='test'
        im_name='{}.jpg'.format(self.frame_num)
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        # if torch.sqrt(self.target_sz.prod()) > 4 * torch.sqrt(self.first_frame_target_sz.prod()):
        #     if torch.sqrt(self.target_sz.prod()) <0.25 * torch.sqrt(self.first_frame_target_sz.prod()):
        use_first_frame_sz=True
        if use_first_frame_sz:
                init_boxes_multi= [init_boxes]
                init_box = self.get_iounet_box(self.pos, self.first_frame_target_sz, sample_pos, sample_scale)
                init_boxes = init_box.view(1, 4).clone()
                if self.params.num_init_random_boxes > 0:
                    # Get random initial boxes
                    square_box_sz = init_box[2:].prod().sqrt()
                    ...
                    init_boxes = torch.cat([new_center - new_sz / 2, new_sz], 1)
                    init_boxes = torch.cat([init_box.view(1, 4), init_boxes])
                init_boxes_multi.append(init_boxes)
                init_boxes=torch.cat(init_boxes_multi)

                # init_boxes_multi= [init_boxes]
                # init_box = self.get_iounet_box(self.pos, self.first_frame_target_sz[[1,0]], sample_pos, sample_scale)
                # init_boxes = init_box.view(1, 4).clone()
                # if self.params.num_init_random_boxes > 0:
                #     # Get random initial boxes
                #     square_box_sz = init_box[2:].prod().sqrt()
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            output_boxes = bb_init + step_length * (bb_init.grad*00).round()/100 * bb_init[:, :, 2:].repeat(1, 1, 2)
    ...
    def get_det_proposals(self):
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            rand_factor = square_box_sz * torch.cat(
                [self.params.box_jitter_pos * torch.ones(2), self.params.box_jitter_sz * torch.ones(2)])
            minimal_edge_size = max(init_box[2:].min() / 3,20)
            rand_bb = (torch.rand(self.params.num_init_random_boxes, 4) - 0.5) * rand_factor
            new_sz = (init_box[2:] + rand_bb[:, 2:]).clamp(minimal_edge_size)
            new_center = (init_box[:2] + init_box[2:] / 2) + rand_bb[:, :2]
            init_boxes = torch.cat([new_center - new_sz / 2, new_sz], 1)
        ...
        init_boxes_multi = [init_boxes]
        init_box = torch.cat(
            (self.last_pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        init_boxes = init_box.view(1, 4).clone()
        if self.params.num_init_random_boxes > 0:
            # Get random initial boxes
            square_box_sz = init_box[2:].prod().sqrt()
        ...
        init_boxes_multi = [init_boxes]
        init_box = torch.cat(
            (self.last_pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]*1.1))
        init_boxes = init_box.view(1, 4).clone()
        if self.params.num_init_random_boxes > 0:
            # Get random initial boxes
            square_box_sz = init_box[2:].prod().sqrt()
        ...
        init_boxes_multi = [init_boxes]
        init_box = torch.cat(
            (self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]*1.1))
        init_boxes = init_box.view(1, 4).clone()
        if self.params.num_init_random_boxes > 0:
            # Get random initial boxes
            square_box_sz = init_box[2:].prod().sqrt()
        ...
        if self.conf_redet > 0.5:
            init_boxes_multi = [init_boxes]
            init_box = torch.cat(
                (self.pos_redet[[1, 0]] - (self.first_frame_target_sz[[1, 0]] - 1) / 2, self.first_frame_target_sz[[1, 0]]))
            init_boxes = init_box.view(1, 4).clone()
            if self.params.num_init_random_boxes > 0:
                # Get random initial boxes
                square_box_sz = init_box[2:].prod().sqrt()
    ...
    def redet(self, img, scale):
        # Compute pre-downsampling factor
        im_sz = torch.round(torch.tensor([img.shape[2] / scale, img.shape[3] / scale]) / 16) * 16

        im = F.interpolate(img, im_sz.long().tolist(), mode='bilinear')

        feat_list = self.params.features.features[0].extract(im)
        self.feature_sz_redet = self.params.features.size(im_sz)
        self.output_sz_redet = im_sz

        if self.params.features.features[0].normalize_power is not None:
            for feat in feat_list:
                feat /= (torch.sum(
                    feat.abs().view(feat.shape[0], 1, 1, -1) ** self.params.features.features[0].normalize_power, dim=3,
                    keepdim=True) /
                         (feat.shape[1] * feat.shape[2] * feat.shape[3]) + 1e-10) ** (
                                    1 / self.params.features.features[0].normalize_power)
        feature_map = TensorList([feat_list]).unroll()

        test_x = self.preprocess_sample(
            operation.conv2d(feature_map, self.projection_matrix).apply(self.projection_activation))

        scores_raw = operation.conv2d(test_x, self.first_frame_filter, mode='same')
        # scores_raw = operation.conv2d(test_x, torch.cat(self.all_filter, dim=0), mode='same')


        self.filters_enough=False


        max_score, max_disp = dcf.max2d(scores_raw[0])
        max_score_filter,filter_ind=max_score.max(dim=1)

        if max_score[0,0]>0.5:
            filter_ind=torch.zeros(1,dtype=torch.long)

        scores_raw[0]=scores_raw[0][:,filter_ind]



        # if scores_raw[0].max()<0.5:
        #     return None, None, None, None
        # elif scores_raw[0].max()<0.7 and filter_ind!=0:
        #     return None,None,None,None


        self.filters_enough = True

        translation_vec, scale_ind, s, flag = self.localize_target_redet(scores_raw)
        if self.params.debug >= 2:
            show_tensor(s[scale_ind, ...], 6,
                        title='redet : Max score = {:.2f}'.format(torch.max(s[scale_ind, ...]).item()))

        conf = torch.max(s[scale_ind, ...])

        sample_pos = torch.tensor([img.shape[2] / 2, img.shape[3] / 2])

        inside_ratio = 0.2
        inside_offset = (inside_ratio - 0.5) * self.first_frame_target_sz

        update_scale_flag = getattr(self.params, 'update_scale_when_uncertain', True) or flag != 'uncertain'
        update_scale_flag =False
        if getattr(self.params, 'use_classifier', True):
            pos = torch.max(torch.min(sample_pos + translation_vec, self.image_sz - inside_offset), inside_offset)
        # self.refine_target_box(sample_pos, scale, scale_ind, update_scale_flag)

        return pos, flag, conf,filter_ind

    def localize_target_redet(self, scores_raw):
        # Weighted sum (if multiple features) with interpolation in fourier domain
        weight = self.fparams.attribute('translation_weight', 1.0)
        scores_raw = weight * scores_raw
        sf_weighted = fourier.cfft2(scores_raw) / (scores_raw.size(2) * scores_raw.size(3))
        for i, (sz, ksz) in enumerate(zip(self.feature_sz_redet, self.kernel_size)):
            sf_weighted[i] = fourier.shift_fs(sf_weighted[i],
                                              math.pi * (1 - torch.Tensor([ksz[0] % 2, ksz[1] % 2]) / sz))

        scores_fs = fourier.sum_fs(sf_weighted)
        scores = fourier.sample_fs(scores_fs, self.output_sz_redet)

        if self.output_window is not None and not getattr(self.params, 'perform_hn_without_windowing', False):
            scores *= self.output_window

        if getattr(self.params, 'advanced_localization', False):
            return self.localize_advanced_redet(scores)

    def localize_advanced_redet(self, scores):
        """Does the advanced localization with hard negative detection and target not found."""
        ...
        max_score1 = max_score1[scale_ind]
        max_disp1 = max_disp1[scale_ind, ...].float().cpu().view(-1)
        target_disp1 = max_disp1 - self.output_sz_redet // 2          #the disp with relative pos
        translation_vec1 = target_disp1 * self.first_frame_target_scale

        target_disp1 = max_disp1 - self.last_pos / self.first_frame_target_scale   #the disp with last pos , and the same as target2
        ...
        # Mask out target neighborhood
        target_neigh_sz = self.params.target_neighborhood_scale * self.first_frame_target_sz / self.first_frame_target_scale
        tneigh_top = max(round(max_disp1[0].item() - target_neigh_sz[0].item() / 2), 0)
        tneigh_bottom = min(round(max_disp1[0].item() + target_neigh_sz[0].item() / 2 + 1), sz[0])
        tneigh_left = max(round(max_disp1[1].item() - target_neigh_sz[1].item() / 2), 0)
        tneigh_right = min(round(max_disp1[1].item() + target_neigh_sz[1].item() / 2 + 1), sz[1])
        ...
        max_score2, max_disp2 = dcf.max2d(scores_masked)
        max_disp2 = max_disp2.float().cpu().view(-1)
        target_disp2 = max_disp2 - self.output_sz_redet // 2
        translation_vec2 = target_disp2 * self.first_frame_target_scale

        target_disp2 = max_disp2 - self.last_pos / self.first_frame_target_scale
    ...
    def tracking_by_detection(self,image,depth_im):
        target_bbox = torch.cat(
            (self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.pos[[1, 0]] + (self.target_sz[[1, 0]] - 1) / 2))
        image_sz = image.shape[0:2]
        # frame_state = BoxList(target_bbox, [image_sz[1], image_sz[0]], mode="xyxy")

        init_boxes = self.get_det_proposals()
        frame_state_multi = BoxList(init_boxes, [image_sz[1], image_sz[0]], mode="xywh").convert("xyxy")
        # self.detector.show_bbox(image[:, :, ::-1], frame_state_multi)

        prediction_target = self.detector.compute_mask_from_box(image[:, :, ::-1], frame_state_multi)
        # prediction = self.detector.compute_mask_from_box(image[:, :, ::-1], frame_state)
        # self.detector.show_prediction(image[:, :, ::-1], prediction_target)

        prediction_det = self.detector.run_on_opencv_image(image[:, :, ::-1])
        self.prediction_det=prediction_det
        # self.detector.show_prediction(image[:, :, ::-1], prediction_det)

        prediction = prediction_det.concat(prediction_target)
        # self.detector.show_prediction(image[:, :, ::-1], prediction)

        prediction = self.detector.remove_easy_distractors(prediction, self.detector.first_frame_distractors_boxes)
        prediction = self.detector.remove_easy_distractors(prediction,self.detector.distractors_boxes)

        # self.detector.show_prediction(image[:, :, ::-1], prediction)

        if self.use_distractor_tracker:
            distractor_states = []
            for i in range(len(self.distractor_trackers)):
                distractor_state = self.distractor_trackers[i].track(image)
                distractor_states.append(distractor_state)
            distractor_states = torch.tensor(distractor_states).reshape(-1, 4)
            distractor_bbox = BoxList(distractor_states, [image_sz[1], image_sz[0]], mode="xywh").convert("xyxy")
            # self.detector.show_bbox(image[:, :, ::-1],distractor_bbox)
            prediction = self.detector.remove_easy_distractors(prediction, distractor_bbox)
            # self.detector.show_prediction(image[:, :, ::-1], prediction)
        self.prediction = prediction

        if prediction is not None and len(prediction) > 0:
            depth_value, mask_area, valid_ratio, outlay_ratio = self.compute_depth_value_and_mask_area(depth_im, prediction)
            # print(depth_value, "      ",depth_sqrt_area, "      ", valid_ratio,"      ",outlay_ratio)

            keep =(depth_value < 10000)  |  np.isnan(depth_value)
            keep=torch.tensor(keep,dtype=torch.bool)
            depth_value=depth_value[keep]
            mask_area=mask_area[keep]
            valid_ratio=valid_ratio[keep]
            outlay_ratio=outlay_ratio[keep]
            prediction = prediction[keep]


            depth_sqrt_area = depth_value * np.sqrt(mask_area)
            # difference_value, depth_rank = torch.sort(
            #     torch.tensor(np.abs(depth_sqrt_area - self.first_frame_depth_sqrt_area)))
            difference_value=torch.tensor(np.abs(depth_sqrt_area - self.first_frame_depth_sqrt_area))

            keep =difference_value < 0.2 * self.first_frame_depth_sqrt_area.item()
            keep=torch.tensor(keep,dtype=torch.bool)
            depth_value=depth_value[keep]
            mask_area=mask_area[keep]
            valid_ratio=valid_ratio[keep]
            outlay_ratio=outlay_ratio[keep]
            prediction_for_depth_refind = prediction[keep]

            aspect_ratios=[]
            for pred in prediction_for_depth_refind:
                _,[L,S], _=self.detector.compute_box_from_mask(pred)
                aspect_ratio=L/S
                aspect_ratios.append(aspect_ratio)
            aspect_ratios=torch.tensor(aspect_ratios)
            keep=  (aspect_ratios/self.detector.first_frame_aspect_ratio  < 1.2)  & (aspect_ratios/self.detector.first_frame_aspect_ratio  > 0.8)
            keep = torch.tensor(keep, dtype=torch.bool)
            depth_value = depth_value[keep]
            mask_area = mask_area[keep]
            valid_ratio = valid_ratio[keep]
            outlay_ratio = outlay_ratio[keep]
            prediction_for_depth_refind = prediction_for_depth_refind[keep]

            if len(prediction_for_depth_refind)==1:
                if (difference_value < 0.35 * self.first_frame_depth_sqrt_area.item()).sum()==1:
                    state, [L, S], flag = self.detector.compute_box_from_mask(prediction_for_depth_refind[0])
                    self.detector.last_S = S
                    self.detector.last_L = L
                    return state
        ...
        disp1 = self.detector.compute_disp(prediction, pos)
        inds = disp1 < 50
        prediction = prediction[inds]
        # todo , should has some condition

        ind = self.detector.choose_prediction(prediction, self.pos, self.target_sz)

        if ind is not None:
            if isinstance(ind, torch.Tensor):
                ind = ind.item()

        if len(prediction) > 0 and ind is not None:
            state, [L, S], flag = self.detector.compute_box_from_mask(prediction[ind])
            self.detector.last_S = S
            self.detector.last_L = L
        else:
            state = None
    ...
    def tracking_by_weak_detection(self, image, depth_im):
        target_bbox = torch.cat(
            (self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.pos[[1, 0]] + (self.target_sz[[1, 0]] - 1) / 2))
        image_sz = image.shape[0:2]
        # frame_state = BoxList(target_bbox, [image_sz[1], image_sz[0]], mode="xyxy")
        ...
        #
        # if prediction is not None and len(prediction) > 0:
        #     depth_value, mask_area, valid_ratio, outlay_ratio = self.compute_depth_value_and_mask_area(depth_im,
        #                                                                                                prediction)
        #
        #     keep = depth_value < 10000
        #
        #     depth_value = depth_value[keep]
        #     mask_area = mask_area[keep]
        #     valid_ratio = valid_ratio[keep]
        #     outlay_ratio = outlay_ratio[keep]
        #     prediction = prediction[torch.tensor(keep)]
        #
        #     if len(prediction) > 0:
        #         depth_sqrt_area = depth_value * np.sqrt(mask_area)
        #         difference_value, depth_rank = torch.sort(
        #             torch.tensor(np.abs(depth_sqrt_area - self.first_frame_depth_sqrt_area)))
        #         # if len(difference_value)>1 and difference_value[1].item()/self.first_frame_depth_sqrt_area<0.2:
        #         #     print("more than one")
        #
        #         if difference_value[0].item() / self.first_frame_depth_sqrt_area < 0.2:
        #             prediction = prediction[depth_rank[0]]
        #
        #             state, [L, S], flag = self.detector.compute_box_from_mask(prediction)
        #             self.detector.last_S = S
        #             self.detector.last_L = L
        #             return state
        #         # print(depth_value, "      ",depth_sqrt_area, "      ", valid_ratio,"      ",outlay_ratio)
        #
        # if self.conf_redet > 0.4:
        #     pos = self.pos_redet
        # else:
        #     pos = self.pos
        #
