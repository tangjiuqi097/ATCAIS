...
class BaseTracker:
    ...
    def track_sequence(self, sequence):
        ...
        # start_frame=650
        # sequence_split=sequence
        # sequence_split.frames=sequence.frames[start_frame:]
        # sequence_split.ground_truth_rect=sequence.ground_truth_rect[start_frame:]
        # sequence_split.init_state=sequence_split.ground_truth_rect[0]
        ...
        self.initialize(image, sequence.init_state,color=sequence.frames[0])
        ...
        if self.params.visualization:
            ...
            self.visualize_cv2(image, sequence.init_state)
        # Track
        tracked_bb = [sequence.init_state]
        for frame in tqdm(sequence.frames[1:]):

            # if int(frame[-8:-4])>=100:
            #     print("only test first 100 frame for debug")
            #     break
            # break
            ...
            start_time = time.time()
            state = self.track(image,color=frame)
            ...
            if self.params.visualization:
                ...
                self.visualize_cv2(image, state)
    ...
    def visualize(self, image, state):
        ...
        rect = patches.Rectangle((state[0], state[1]), state[2], state[3], linewidth=1, edgecolor=edgecolor, facecolor='none')
    ...
    def visualize_cv2(self,image,state):
        frame=image[:,:,::-1]
        bbox = list(map(int, state))

        frame=cv.rectangle(frame, (bbox[0], bbox[1]),
                      (bbox[0] + bbox[2], bbox[1] + bbox[3]), self.edgecolor, 3)
